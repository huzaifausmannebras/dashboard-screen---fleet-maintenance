...
var Parser = /** @class */ (function () {
    ...
    Parser.prototype.addParensToArgs = function (fSubstr) {
        ...
        if (id === -1) {
            ...
            while (id > -1) {
                var fCheck = fSubstr.substring(id + 1, fSubstr.length);
                var j = this.indexOfAny(fCheck, rightSides);
                var formulaVal = this.indexOfAny(fCheck, ['q']);
                var fId = fCheck.indexOf(this.parent.getParseArgumentSeparator());
                var marker = ['+', '-', '*', '/'];
                if (j >= 0) {
                    j = id + j + 1;
                }
