...
function createLabelTemplate(template, heatMap, labelTemplate, rectPosition, xLabels, yLabels, index) {
    ...
    if (heatMap.dataSourceSettings.isJsonData && (heatMap.dataSourceSettings.adaptorType === 'Cell' || heatMap.dataSourceSettings.adaptorType === 'Table')) {
        ...
        dataSource.forEach((item) => {
            ...
            if (Object.keys(item).some((key) => item[key] === xLabelData[rectPosition.xIndex]) && yDataMapping) {
                rectData = item;
            }
        });
    }
            ...
            if ((axis.textStyle.textOverflow === 'Wrap' || isLineBreak) && !axis.enableTrim) {
                ...
                if (axis.orientation === 'Vertical' || (axis.orientation === 'Horizontal' && axis.angle % 180 !== 0)) {
                    ...
                    for (let index = 0; index < wrappedlabels.length; index++) {
                        let wrappedlabelSize = measureText(wrappedlabels[index], axis.textStyle);
                ...
                if (axis.orientation === 'Vertical') {
                    let tempintervel = heatmap.initialClipRect.height / (axis.axisLabelSize / axis.axisLabelInterval);
                    for (let index = 0; index < wrappedlabels.length; index++) {
...
class AxisHelper {
    ...
    drawXAxisLabels(axis, parent, rect) {
        ...
        for (let i = 0, len = labels.length; i < len; i++) {
            ...
            const options = new TextOption(heatMap.element.id + '_XAxis_Label' + i, new TextBasic(x, y, (angle % 360 === 0) ? axis.textStyle.textAlignment === 'Center' ? 'middle' : 'start' : 'middle', label, angle, transform), axis.textStyle, axis.textStyle.color || heatMap.themeStyle.axisLabel);
            /* eslint-disable max-len */
...
class ExportUtils {
    ...
    export(type, fileName, orientation) {
        ...
        if (type === 'SVG') {
            ...
            let backgroundColor = ctx.shadowColor;
            image.onload = (() => {
